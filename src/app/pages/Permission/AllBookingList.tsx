...
function AllBookingList() {
  ...
  const headers = [
    ...
    { name: 'Track', field: 'track', sortable: false },
  ]
  ...
  const getAllData = async (page: any) => {
    ...
    dispatch(adminAction.getCurrentBookings(query));
  };

  const handleRouterToMap = (item: any) => {
    localStorage.setItem("current_driver", JSON.stringify(item));
    navigate(`/live-tracking/${item.driver_id}`);
  };
