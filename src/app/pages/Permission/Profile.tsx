...
function Profile() {
  ...
  const get_product_list: any = useSelector((state: any) =>
    state.admin.profile_list ? state.admin.profile_list : {}
  )
  ...
  const handlePageClick = (event: any) => {
    ...
    // if (!userSearch) {
    // dispatch(adminAction.getAllProfile(data))
  ...
  const hendleSubmitCreate = async () => {
    ...
    try {
      ...
      if (!roledata?.name) {
        ...
        user?.data?.circle_id && data.append('circle_id', user?.data?.circle_id)
        user?.data?.xen_id && data.append('xen_id', user?.data?.xen_id)
        user?.data?.aen_id && data.append('aen_id', user?.data?.aen_id)
        user?.data?.jen_id && data.append('jen_id', user?.data?.jen_id)
        user?.data?.manager_id && data.append('manager_id', user?.data?.manager_id)
        ...
        await dispatch(adminAction.createProfile(data))
        await dispatch(adminAction.getAllProfile({ page: pageNo, search }))
        setShow(false)
        setLoading(false)
      }
  ...
  useEffect(() => {
    dispatch(adminAction.getAllProfile({ page: pageNo, search }))
  }, [pageNo, search])
  ...
  const hendleDelete = async () => {
    // deleteProduct
    setShowDelete(false)
    await dispatch(adminAction.deleteProfile({ id: productID, page: pageNo }))
    await dispatch(adminAction.getAllProfile({ page: pageNo, search }))
    // dispatch(adminAction.getRoleList('dhsg'))
    return () => { }
  }
  ...
  const hendleSubmitupdateperentCate = async () => {
    setLoading(true)
    try {
      // const {data: auth} = await login(values.role_name, values.role_description)
      // saveAuth(auth)
      ...
      await dispatch(adminAction.assignUserProfile(data))
      setShowUpdate(false)
      setLoading(false)
      await dispatch(adminAction.getAllProfile({ page: pageNo, search }))
      //   }
  ...
  const hendleSubmitUpdate = async () => {
    // setLoading(true)
    try {

      const newFrom = Object.keys(updateData)
        .filter((key) => updateData[key] !== roledata[key])
        .reduce((acc, key) => ({ ...acc, [key]: updateData[key] }), {});

      if (Object.keys(newFrom).length > 0) {

        await dispatch(adminAction.updateProfile({
          id: updateData?.id,
          ...newFrom
        }));
      ...
      <div className='d-flex flex-column flex-column-fluid'>
        ...
        <div id='kt_app_toolbar' className='app-toolbar py-3 py-lg-6'>
          ...
          >
            ...
            <Modal show={show1} dialogClassName='modal-90w' onHide={handleClose1}>
                    ...
                    >
                      ...
                      {get_role_list &&
                        get_role_list?.map((data, i) => {
                          console.log(data?.id, role_id);

                          if (role_id < data?.id && data?.id <= 8) {
                            return (
