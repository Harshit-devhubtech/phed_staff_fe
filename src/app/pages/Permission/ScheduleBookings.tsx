...
function ScheduleBookings() {
  ...
  const headers = [
    ...
    { name: 'Vendor', field: 'vendor_data', sortable: false },
    { name: 'Driver', field: 'driver', sortable: false },
    { name: 'Status', field: 'status', sortable: false },
    { name: 'Days', field: 'Days', sortable: false },
    { name: 'Days Change', field: 'change', sortable: false },
  ]
  ...
  const handlePageClick = (event: any) => {
    ...
    // if (!userSearch) {
    dispatch(adminAction.getScheduleBookings(data))
  ...
  useEffect(() => {
    ...
    dispatch(adminAction.getScheduleBookings(1))
  ...
  const hendleDelete = () => {
    // deleteProduct
    setShowDelete(false);
    dispatch(adminAction.deleteScheduleBooking(itemId, pageNo));
    // dispatch(adminAction.getRoleList('dhsg'))
    return () => { }
  };
  ...
  const hendleStatusUpdate = (id: any, status: any) => {
    ...
    dispatch(adminAction.updateScheduleBooking(data, pageNo, 'cu'))
    return () => { }
  };
  ...
  const hendletochangeDriver3 = () => {
    ...
    data.append('id', bookingdata?.id)
    data.append('sunday', bookingdata?.sunday)
    data.append('monday', bookingdata?.monday)
    data.append('tuesday', bookingdata?.tuesday)
    data.append('wednesday', bookingdata?.wednesday)
    data.append('thursday', bookingdata?.thursday)
    data.append('friday', bookingdata?.friday)
    data.append('saturday', bookingdata?.saturday)
 ...
 directionsService.getDistanceMatrix(
  ...
  const loadOptionsgetManagerVehicle = async (
    ...
    // roledataUpdate
    console.log(bookingdata?.vehicle?.aen_id)
    ...
    const response = await fetch(
      URL.API_BASE_URL +
      prfix +
      `/all_vehicle_jen?search=${search}&page=${page}&jen_id=${bookingdata?.vehicle?.jen_id}`,
      {
        headers: {
          Authorization: `Bearer ${JSON.parse(token).api_token}`,
        },
  ...
  const loadOptionsgetManagerJen = async (search: any, loadedOptions: any, { page }: { page: any }) => {
    ...
    const response = await fetch(
      URL.API_BASE_URL +
      prfix +
      `/getSource?search=${search}&page=${page}&jen_id=${bookingdata?.vehicle?.jen_id}`,
      {
        headers: {
          Authorization: `Bearer ${JSON.parse(token).api_token}`,
        },
      ...
      <div className='d-flex flex-column flex-column-fluid'>
        ...
        <div id='kt_app_toolbar' className='app-toolbar py-3 py-lg-6'>
          ...
          <div id='kt_app_toolbar_container' className='d-flex flex-stack container-xxl'>
            ...
            <Modal show={showUpdate1} dialogClassName='' onHide={handleCloseUpdate1} size="lg">
              <Modal.Header closeButton>
            ...
            <Modal show={showUpdate3} onHide={handleCloseUpdate3}>
              <Modal.Header closeButton>
                <Modal.Title>Change Days </Modal.Title>
              </Modal.Header>
              <Modal.Body>
                {/* Woohoo, you're reading this text in a modal! */}
                {/* <form
                    ...
                    {commentsData?.map((data: any, i: any) => {
                      ...
                      return (
                        <tr key={i} style={{ backgroundColor: `${colorPicker(data?.status)}` }}>
                          ...
                          <td style={{ color: 'red' }}>
                            {prefixdata == 'vendors' ? '--' : data?.otp}
                          </td>
                          <td>
                            {data?.vehicle?.registration_number}
                            {(prefixdata == 'jens' || prefixdata == 'aens') && (
                              <button
                                data-id='2209'
                                onClick={() => {
                          ...
                          <td style={{ 'textAlign': 'center' }}> {data?.vendor_data?.name || "--"}</td>
                          <td>
                            {data?.driver?.name}
                            {(prefixdata == 'jens' || prefixdata == 'aens') && (
                              <button
                          ...
                          <td>
                            <button
                              data-id='2209'
                              onClick={() => {
                                const statusValue = data?.active ? 0 : 1
                                hendleStatusUpdate(data?.id, statusValue)
                              }}
                              className={
                                data?.active
