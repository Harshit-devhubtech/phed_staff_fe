...
function Destination() {
  ...
  const get_product_list: any = useSelector((state: any) =>
    state.admin.destionation_Details ? state.admin.destionation_Details : {}
  )
  ...
  const headers = [
    ...
    { name: 'Pin code ', field: 'pincode', sortable: false },
  ...
  const handlePageClick = (event: any) => {
    ...
    // if (!userSearch) {
    // dispatch(adminAction.get_user_destination(data))
  ...
  const hendleSubmitCreate = async () => {
    ...
    try {
      ...
      if (!roledata?.city) {
        ...
        await dispatch(adminAction.user_destination(data))
        setcecleId('')
        setShow(false)
        setLoading(false)
        await dispatch(adminAction.get_user_destination({ page: pageNo, search }))
      }
  ...
  useEffect(() => {
    ...
    // dispatch(adminAction.get_user_destination(1))
  ...
  useEffect(() => {
    dispatch(adminAction.get_user_destination({ page: pageNo, search }))
  }, [pageNo, search])
  ...
  const loadOptionsRole = async (search: any, loadedOptions: any, { page }: { page: any }) => {
    ...
    const response = await fetch(
      URL.API_BASE_URL + prfix +
      `/get_all_user_circle?search=${search}&page=${page}&circle_id=${criId}`,
      {
        headers: {
          ...
          >
            ...
            <div className='d-flex flex-column flex-wrap justify-content-center me-3 page-title'>
              ...
              <ul className='my-0 breadcrumb-separatorless pt-1 fw-semibold breadcrumb fs-7'>
                ...
                {/* <!--end::Item-->  */}
                {/* <!--begin::Item-->  */}
                <li className='breadcrumb-item text-muted'> Destination</li>
                {/* <!--end::Item-->  */}
              </ul>
              {/* <!--end::Breadcrumb-->  */}
            </div>
