...
function Rate() {
  ...
  const get_product_list: any = useSelector((state: any) =>
    state.admin.get_rate_list ? state.admin.get_rate_list : {}
  )
  ...
  const headers = [
    ...
    { name: 'kmFrom', field: 'kmFrom', sortable: true },
    { name: 'kmTo', field: 'kmTo', sortable: true },
  ...
  const handlePageClick = (event: any) => {
    ...
    // if (!userSearch) {
    // dispatch(adminAction.getrate(data))
  ...
  const hendleSubmitCreate = () => {
    ...
    try {
      ...
      if (!roledata?.role_name) {
        ...
        toast.error('please select circle_id')
      } else if (!roledata?.kmFrom) {
        toast.error('please enter kmFrom')
        ...
        data.append('kmFrom', roledata?.kmFrom)
        ...
        dispatch(adminAction.createrate(data))
        setShow(false)
        setLoading(false)
        dispatch(adminAction.getrate({page:pageNo,search}))
      }
      // setCurrentUser(user)
    } catch (error) {
      // console.error(error)
  ...
  useEffect(() => {
    ...
    // dispatch(adminAction.getrate(1))
  ...
  const hendleDelete = () => {
    // deleteProduct
    setShowDelete(false)
    dispatch(adminAction.deleteRaye(productID))
    // dispatch(adminAction.getRoleList('dhsg'))
    dispatch(adminAction.getrate({page:pageNo,search}))
    return () => { }
  }
  ...
  const hendleSubmitupdateperentCate = () => {
    ...
    try {
      ...
      if (!roledataUpdate?.name) {
      ...
      } else if (!roledataUpdate?.kmTo) {
        toast.error('please enter kmTo')
      } else if (!startDateup) {
        toast.error('please enter effectiveDate')
      } else if (!roledataUpdate?.price) {
        toast.error('please enter price')
      } else {
        data.append('name', roledataUpdate?.name)
        // data.append('description', categoryUpPage)
        data.append('circle_id', roledataUpdate?.circle_id)
        data.append('kmFrom', roledataUpdate?.kmFrom)
        ...
        dispatch(adminAction.updaterate(roledataUpdate))
        setShowUpdate(false)
        setLoading(false)
        dispatch(adminAction.getrate({page:pageNo,search}))
      }
          ...
          >
            <Modal show={show} onHide={handleClose}>
              ...
              <Modal.Body>
                ...
                <div className='fv-row mb-7'>
                  <label className='fs-6 fw-semibold form-label mb-2'>
                    <span className='required'>kmFrom</span>
                    <i
                ...
                <div className='fv-row mb-7'>
                  <label className='fs-6 fw-semibold form-label mb-2'>
                    <span className='required'>kmTo</span>
                    <i
                      className='fas fa-exclamation-circle ms-2 fs-7'
                      data-bs-toggle='popover'
                      data-bs-trigger='hover'
            ...
            <div className='page-title d-flex flex-column justify-content-center flex-wrap me-3'>
              ...
              <ul className='breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1'>
                ...
                {/* <!--end::Item-->  */}
                {/* <!--begin::Item-->  */}
                <li className='breadcrumb-item text-muted'>Rate</li>
                {/* <!--end::Item-->  */}
              </ul>
              {/* <!--end::Breadcrumb-->  */}
            </div>
