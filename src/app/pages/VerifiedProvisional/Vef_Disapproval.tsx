...
const Vef_Disapproval = () => {
  ...
  const getReports = async () => {
    ...
    dispatch(adminAction.getReportsFilter(pageNo, query, "reject"));
  };
            ...
            <tbody style={{ maxHeight: '60vh', overflow: 'auto' }}>
              {reportData.length > 0 &&
                reportData.map((item: any, index: any) => (
                  <tr key={index}
                    style={{ backgroundColor: '#E54236', color: 'white' }}
                  >
                    <td>{index + 1}</td>
                    <td>{item?.id}</td>
                    ...
                    <td>{item?.remark}</td>
                    <td>
                      <Link to={`/view_details/${item?.id}/reject`}>
...
export default Vef_Disapproval
